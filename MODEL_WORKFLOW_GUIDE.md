...
### 3-4. SHAP分析結果の見方（詳細解説）

#### 📈 Summary Plot（最重要！）

**Summary Plotは最も重要なグラフです。** このグラフ1つでモデル全体の挙動が理解できます。

##### 🎨 **色の意味を理解する**

**色 = 特徴量の値の大きさ**
- 🔴 **赤（ピンク）**: 特徴量の値が**高い**（High value）
- 🔵 **青**: 特徴量の値が**低い**（Low value）
- ⚫ **グレー**: 特徴量の値が**中間**（Medium value）

**横軸 = SHAP値（予測への影響）**
- **右側（プラス）**: 着順を**良くする**影響（1着に近づける）
- **左側（マイナス）**: 着順を**悪くする**影響（下位に落とす）
...
**ステップ1: 最重要特徴量を特定**
```
一番上の特徴量 = モデルが最も重視している要素
例: past_avg_sotai_chakujun（過去平均相対着順）
```
...
**パターンA: 青が左、赤が右（正の相関）**
```
例: kishu_surface_score（騎手路面スコア）
🔵●●●● | ●●●●🔴
  左側  0  右側

解釈: 値が高い（赤）ほど着順が良くなる（右側）
→ 路面が得意な騎手ほど好走する
```
...
```
例: past_avg_sotai_chakujun（過去平均相対着順）
※注意: この特徴量は実際には「青が左、赤が右」パターンになっている

実際の分布:
🔵●●●● | ●●●●🔴
  左側  0  右側

⚠️ 問題発見: 値が低い（青）=過去成績が良い馬が左側（マイナス影響）
→ 「過去成績が良いほど予測着順を悪くする」という逆効果！
→ 特徴量の符号を反転させる必要あり
```

**パターンC: 色が混在（複雑な関係）**
```
例: umaban_kyori_interaction（馬番×距離）
🔵🔴●● | ●●🔴🔵
  左側  0  右側
...
- **広く散らばる** → 影響度が大きい（重要）
- **狭く集まる** → 影響度が小さい（重要度低）
- **縦に密集** → ほとんどの馬で同じ値（特徴量設計の問題）

##### ⚠️ **よくある誤解と正しい解釈**

❌ **誤解1**: 「赤い点が多い = 重要な特徴量」
✅ **正解**: 色は値の大小を示すだけ。重要度は**縦軸の順番**と**点の広がり**で判断。

❌ **誤解2**: 「右側にある = 良い特徴量」
✅ **正解**: 右側（プラス）は着順を上げる影響。左側（マイナス）も重要（着順を下げる要因を知ることも大事）。

❌ **誤解3**: 「青と赤が混ざってる = 悪い特徴量」
✅ **正解**: 複雑な関係性を捉えている可能性あり。Dependence Plotで詳細確認が必要。

##### 🎯 **実践例: あなたのSummary Plotを読む**

```
【Top 3特徴量の解釈】

1. past_avg_sotai_chakujun（過去平均相対着順）
   🔵が左、🔴が右 → 過去の成績が良いほど今回も好走
   広がりが最大 → 最重要特徴量
   ⚠️ 赤い点が多すぎる → 過去成績が悪い馬が多い（データ品質の問題？）

2. kishu_surface_score（騎手路面スコア）
   🔵が左、🔴が右 → 路面が得意な騎手ほど好走
   広がりは中程度 → 重要だが1位ほどではない

3. umaban_kyori_interaction（馬番×距離）
   色が混在 → 単純な線形関係ではない
   → Dependence Plotで詳細分析が必要
```

#### 📊 Bar Plot（重要度ランキング）

**Bar Plotは特徴量の重要度を一目で比較できます。**

**見るべきポイント:**
- **横軸**: 平均絶対SHAP値（大きいほど重要）
- **縦軸**: 特徴量名（上から重要度順）
...
Middle特徴量（0.01 ≤ SHAP < 0.05）:
  → 補助的な役割
  → 改良の優先度は中

Bottom特徴量（SHAP < 0.005）:
  → 削除候補
  → モデルがほとんど使っていない
```
...
**判断基準の目安:**
- **SHAP ≥ 0.1**: 超重要（モデルの主柱）
- **0.05 ≤ SHAP < 0.1**: 重要（補助的に貢献）
- **0.01 ≤ SHAP < 0.05**: やや重要（微調整用）
- **0.005 ≤ SHAP < 0.01**: 微影響（削除検討）
- **SHAP < 0.005**: 不要（削除推奨）

#### 🔗 Dependence Plot（相関関係）

**Dependence Plotは特徴量と予測の関係を詳しく見るグラフです。**
...
解釈: 値が大きいほど着順が良くなる（単純な正の相関）
例: kishu_surface_score（騎手が得意なほど好走）
```
...
解釈: 値が大きいほど着順が悪くなる（単純な負の相関）
例: past_avg_sotai_chakujun（過去成績が悪いほど今回も悪い）
```
...
解釈: 非線形な関係（閾値がある、相互作用が強い）
→ 特徴量の設計を見直す余地あり
```

**色の意味（相互作用）:**
- 点の色が**グラデーション**になっている → 他の特徴量との相互作用あり
- 例: 馬番の効果が距離によって変わる

#### ⚡ Force Plot（個別レース分析）

**Force Plotは1レースの予測理由を詳しく見るグラフです。**

**見るべきポイント:**
- **赤いバー**: プラス影響（着順を上げる要因）
- **青いバー**: マイナス影響（着順を下げる要因）
- **バーの長さ**: 影響の大きさ
...
**的中したレースの分析:**
```
赤いバーが長い特徴量 → この馬の強みだった
例: past_avg_sotai_chakujun(赤・長) → 過去成績が決め手
```

**外れたレースの分析:**
```
青いバーが長い特徴量 → この馬の弱点だった
例: kishu_surface_score(青・長) → 騎手が路面苦手だった
```
...
予想: 1着だったが実際は5着
→ Force Plotで確認
→ past_score(赤)は良かったが、umaban(青)が大きくマイナス
→ 馬番が悪すぎた（外枠不利）
```
