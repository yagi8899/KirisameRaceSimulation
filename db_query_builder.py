...
def build_race_data_query(
    ...
    FROM (
        ...
        CASE 
            ...
            ELSE 0
        END AS rest_days_fresh{payout_columns}
    from
        jvd_ra ra 
        inner join ( 
            select
...
def build_sokuho_race_data_query(
    ...
    FROM (
        ...
        CASE 
            WHEN COUNT(seum.kakutei_chakujun) OVER (
                ...
                (
                    ...
                    FROM (
                        SELECT 
                            (1.0 - cast(seum2.kakutei_chakujun as float) / NULLIF(cast(seum2.shusso_tosu as float), 0))
                            * CASE
                                WHEN seum2.time_sa LIKE '-%' THEN 1.00
                                WHEN CAST(REPLACE(seum2.time_sa, '+', '') AS INTEGER) <= 5 THEN 0.85
                                WHEN CAST(REPLACE(seum2.time_sa, '+', '') AS INTEGER) <= 10 THEN 0.70
                        ...
                        FROM (
                            SELECT se2.kakutei_chakujun, se2.time_sa, past_ra2.shusso_tosu, se2.kaisai_nen, se2.kaisai_tsukihi
                            FROM jvd_se se2
                            INNER JOIN jvd_ra past_ra2 
                                ON se2.kaisai_nen = past_ra2.kaisai_nen
                                AND se2.kaisai_tsukihi = past_ra2.kaisai_tsukihi
                                AND se2.keibajo_code = past_ra2.keibajo_code
                                AND se2.race_bango = past_ra2.race_bango
                            WHERE se2.ketto_toroku_bango = seum.ketto_toroku_bango
                                AND (
                                    cast(se2.kaisai_nen as integer) < cast(seum.kaisai_nen as integer)
                                    OR (
                                        cast(se2.kaisai_nen as integer) = cast(seum.kaisai_nen as integer)
                                        AND cast(se2.kaisai_tsukihi as integer) < cast(seum.kaisai_tsukihi as integer)
                                    )
                                )
                            ORDER BY cast(se2.kaisai_nen as integer) DESC, cast(se2.kaisai_tsukihi as integer) DESC
                            LIMIT 5
                        ) seum2
                    )
                )
                ...
                (
                    ...
                    FROM (
                        SELECT 
                            ...
                            AVG(cast(seum2.kakutei_chakujun as float)) OVER () AS avg_chaku
                        FROM (
                            SELECT se2.kakutei_chakujun, se2.kaisai_nen, se2.kaisai_tsukihi
                            FROM jvd_se se2
                            WHERE se2.ketto_toroku_bango = seum.ketto_toroku_bango
                                AND (
                                    cast(se2.kaisai_nen as integer) < cast(seum.kaisai_nen as integer)
        ...
        -- 4. zenso_kakoi_komon: 前走囲い込み度（前走2コーナー - 前走4コーナー）
        LAG(
            CASE 
                WHEN seum.corner_2 IS NOT NULL AND seum.corner_2 <> '' 
                    AND seum.corner_4 IS NOT NULL AND seum.corner_4 <> ''
