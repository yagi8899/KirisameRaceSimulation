...
def analyze_upset_threshold():
    ...
    try:
        ...
        if len(upset_horses) > 0:
            print(f"[UPSET] 人気範囲: {upset_horses['ninki'].min()}〜{upset_horses['ninki'].max()}番人気")
            
            # 確率列があるかチェック
            prob_cols = [col for col in df.columns if 'prob' in col.lower() or 'upset' in col.lower()]
            print(f"[DEBUG] 確率関連列: {prob_cols}")
            ...
            # 穴馬の統計情報
            print(f"\n[UPSET] 穴馬詳細:")
            for idx, row in upset_horses.iterrows():
                print(f"  レース{row.get('race_id', 'N/A')} {row['ninki']:2d}番人気 → {row['kakutei_chakujun']}着")
        else:
            print("[UPSET] 穴馬が0頭！これが問題の原因")
        ...
        print(f"\n[STATS] 人気別着順分布:")
        for ninki in range(7, 13):
            horses_in_ninki = df[df['ninki'] == ninki]
            if len(horses_in_ninki) > 0:
                in_top3 = len(horses_in_ninki[horses_in_ninki['kakutei_chakujun'] <= 3])
                print(f"  {ninki:2d}番人気: {len(horses_in_ninki):3d}頭中 {in_top3:2d}頭が3着以内 ({in_top3/len(horses_in_ninki)*100:.1f}%)")
